#!/usr/bin/env python3
"""wake_monitor.py — the Loop-Mode wake-monitor (the missing companion to
modules/wake-monitor.md).
...
Surfaces watched (the directive surfaces from modules/wake-monitor.md §contract):
...
Env:
  SHIP_ROOT       ship root (default: parent of this scripts/ dir)
...
SHIP_ROOT = Path(os.environ.get("SHIP_ROOT", HERE.parent)).resolve()
...
# Import the canonical classifier so a declared `wake_class` is honored verbatim
# (no second, drifting copy of the wake/batch/silent ladder lives here).
