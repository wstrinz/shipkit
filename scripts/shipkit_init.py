...
THREE-WAY PATH DISTINCTION (common confusion point):
  ship_root  — where ship STATE physically lives (queue.md, captain.md,
               projects/, logs/, state/). Must be the ship directory itself,
               e.g. /Users/you/dev/ship — NOT the parent dev/. Every skill
               resolves all ship paths from here; a wrong ship_root breaks
               the loop. See loop.config.example.json → "_ship_root" for the
               field contract.
  repos      — the repos crew work in. Used for the preflight git-status
               sweep and for orienting dispatch. Has nothing to do with where
               ship state lives. See loop.config.example.json → "_repos".
  launch-cwd — optionally launch Claude Code from the parent dev/ so sibling
               repos are filesystem-visible (e.g. you can open ../my-app from
               within the Mate's session). This is a visibility convenience,
               NOT a ship_root. ship_root is always the ship dir itself.
...
def build_config(answers: dict) -> dict:
    ...
    cfg = {
        "_comment": (
            ...
            "KEY DISTINCTION: ship_root = where ship STATE lives (queue.md, captain.md, "
            "projects/, logs/, state/) — must be the ship dir, NOT the parent dev/. "
            "repos = repos crew work in (preflight git sweep). "
            "launch-cwd = where you cd before launching Claude Code (optionally the "
            "parent dev/ for sibling-repo visibility) — scoped to launch only, "
            "never set ship_root to the parent."
        ),
...
def write_ship_root_pointer(ship_root: str, dry_run: bool) -> list[str]:
    """Write ~/.claude/ship-root.txt so ship-watch-start can locate loop.config.json
    without a prior 'find' search. A one-line absolute path, no trailing newlines
    beyond the line terminator. Idempotent — overwrites if stale."""
    pointer = Path.home() / ".claude" / "ship-root.txt"
    abs_root = str(Path(ship_root).expanduser().resolve()) if ship_root != "." else str(SHIPKIT_ROOT)
    if dry_run:
        return [f"would write ~/.claude/ship-root.txt -> {abs_root!r}"]
    try:
        pointer.parent.mkdir(parents=True, exist_ok=True)
        pointer.write_text(abs_root + "\n", encoding="utf-8")
        return [f"wrote ~/.claude/ship-root.txt -> {abs_root!r}"]
    except OSError as e:
        return [f"WARNING: could not write ~/.claude/ship-root.txt ({e}) — "
                "ship-watch-start will need ship_root from loop.config.json directly"]
...
def smoke_test_lines(module_list: list[str], skills_target: Path) -> list[str]:
    ...
    lines = [
        "",
        "Acceptance gate — work through these in order:",
        "",
        "  LAUNCH SEQUENCE",
        "  1. Open Claude Code in the shipkit dir (e.g. 'cd .../dev/ship', then",
        "     launch). Note: you may optionally launch from the parent dev/ folder",
        "     for sibling-repo filesystem visibility — that is a LAUNCH-CWD choice,",
        ...
        "     message if chat_surface is configured). The loop MUST wake within",
        "     seconds and run a tick. If it does not wake, the wake-monitor or",
        "     chat_surface config is wrong — stop and debug before continuing.",
        "",
        "  BOOKKEEPING DOES NOT WAKE (must also be true)",
        "  5. Make a bookkeeping-only change: update a status field in queue.md or",
        "     state/status.json without any directive text. The loop must NOT wake;",
        "     the change appears reconciled at the next scheduled tick. (This",
        "     asymmetry is the input model working correctly.)",
        "",
        "  QUIET TICK CLOSES THE LOOP",
        "  6. Wait for one quiet tick. Confirm: (a) a telemetry line appears in",
        "     today's logs/mate/<date>.md, and (b) state/status.json was updated.",
        "     ship-watch-start exits; ship-tick self-paces from here.",
    ]
    if has_surface:
        lines += [
            "",
...
def main() -> None:
    ...
    plan.append("== ~/.claude/ship-root.txt (bootstrap pointer) ==")
    plan += [f"  {ln}" for ln in write_ship_root_pointer(
        answers.get("ship_root", "."), args.dry_run)]
